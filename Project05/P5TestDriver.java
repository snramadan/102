...
public class P5TestDriver
{
   ...
   public static void main(String[] args) throws ClassNotFoundException
   {
      ...
      pass &= testGetNearEndForPerformance();
      pass &= testAddNearEndForPerformance();
   ...
   private static boolean testSizeForOrderOnePerformance()
   {
      ...
      System.out.println("Expect < 10000, found " + time);

      pass &= test(time < 10000,
         "\nPerformance does not appear to be O(1) as required\n"
       + "NOTE: This test is designed to run on a CSL server. If you are not\n"
       + "      running on a CSL server you should do so now. If you are\n"
       + "      running on a CSL server the slown speed could be due to\n"
   ...
   private static boolean testGetNearEndForPerformance()
   {
      System.out.println("Testing get() near the end of the list for performance...");
      System.out.println("   If running from console <cntl>-c will terminate");
      boolean pass = true;  
      double sum = 0;
      ...
      System.out.println("Expect < 20000, found " + time);

      pass &= test(time < 20000,
         "\nGet() from over halfway in the list does not appear to use tail to be efficient.\n"
       + "NOTE: This test is designed to run on a CSL server. If you are not\n"
       + "      running on a CSL server you should do so now. If you are\n"
       + "      running on a CSL server the slown speed could be due to\n"
       + "      system load - try testing again on a different CLS server.");
   ...
   private static boolean testAddNearEndForPerformance()
   {
      System.out.println("Testing add() near the end of the list for performance...");
      System.out.println("   If running from console <cntl>-c will terminate");
      boolean pass = true;  
      double sum = 0;
      ...
      long start = System.nanoTime();
      ll.add(199994, (long)500);
      double time = System.nanoTime() - start;

      System.out.println("Expect < 100000, found " + time);

      pass &= test(time < 100000,
         "Add() from over halfway in the list does not appear to use tail to be efficient.\n"
       + "NOTE: This test is designed to run on a CSL server. If you are not\n"
       + "      running on a CSL server you should do so now. If you are\n"
       + "      running on a CSL server the slown speed could be due to\n"
       + "      system load - try testing again on a different CLS server.");
